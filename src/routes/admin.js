...
router.post('/recompute-scores', requireAdmin, async (req, res) => {
  try {
    const { rows: products } = await db.query(
      `SELECT DISTINCT pf.product_id, p.is_featured, p.priority
       FROM product_features pf
       JOIN products p ON p.id = pf.product_id`
    );
    let updated = 0;
    for (const { product_id, is_featured, priority } of products) {
      ...
      const result = calculateMatchScore(features, { isFeatured: is_featured, priority });
      await db.query(
...
// GET /api/admin/site-users — registered site users
...
// PATCH /api/admin/site-users/:id/toggle — toggle is_active
router.patch('/site-users/:id/toggle', requireAdmin, async (req, res) => {
  try {
    const { rows } = await db.query(
      `UPDATE site_users SET is_active = NOT is_active
       WHERE id = $1
